...
router.post('/', auth, multerUpload, async (req, res) => {
  try {
    ...
    // Auto-generate image for ALL ads posted without photos (fire and forget)
    if (!_normalizedAd.images || _normalizedAd.images.length === 0) {
      generateAIImage(ad); // fire and forget — updates ad in background
    }
