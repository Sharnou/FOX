...
router.post('/', auth, async (req, res) => {
  try {
    ...
    const ad = await Ad.create({ userId: req.user.id, seller: req.user.id, title: `${menuItem} — ${restaurantName}`, description: `Delivery: ${deliveryTime} min`, city, country: req.user.country, category: 'Fast Food', subcategory: 'Restaurant', price, media });
