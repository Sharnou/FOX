      ...
      return (
        <li key={item + idx}>
          <button
            ...
            className={'w-full flex items-center gap-3 px-4 py-2.5 text-sm transition-colors ' + (isRtl ? "text-right flex-row-reverse" : "text-left") + ' ' + (isActive
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="relative flex items-center">
        <input
          ...
          className={'w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm shadow-sm outline-none ring-0 transition-all focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 ' + (isRtl ? "text-right pr-12 pl-4 font-cairo" : "text-left pl-12 pr-4")}
          autoComplete="off"
          spellCheck={false}
        />
        ...
        {query.length > 0 && (
          <button
            ...
            className={'absolute top-1/2 -translate-y-1/2 text-xs text-gray-400 hover:text-gray-600 px-2 py-0.5 rounded ' + (isRtl ? "left-2" : "right-2")}
          >
            {t.clear}
          </button>
        )}
