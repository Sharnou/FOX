  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', padding: 16, fontFamily: "'Cairo', 'Tajawal', system-ui, sans-serif" }}>
      ...
      {ad.video ? (
        ...
        <img src={getAdDefaultImage(ad)} alt={ad.title} style={{ width: '100%', height: 200, objectFit: 'cover', borderRadius: 12 }} />
      )}
      ...
      {/* WhatsApp button — forced fix #208 */}
      {(phone || (ad && ad.seller?.phone)) && (
        <a
          href={`https://wa.me/${((phone || (ad && ad.seller?.phone) || '').replace(/[^0-9]/g, ''))}?text=${encodeURIComponent('مرحباً، رأيت إعلانك على XTOX: ' + ((ad && ad.title) || ''))}`}
          target="_blank"
          rel="noopener noreferrer"
          style={{ display: 'block', width: '100%', padding: '14px', marginTop: '8px', background: '#25D366', color: 'white', fontWeight: 'bold', fontSize: '16px', borderRadius: '12px', textAlign: 'center', textDecoration: 'none', boxSizing: 'border-box' }}
        >
          🟢 تواصل عبر واتساب
        </a>
      )}
